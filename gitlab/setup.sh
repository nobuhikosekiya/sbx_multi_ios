...
docker-compose exec gitlab /bin/bash -c "echo external_url \'http://127.0.0.1\' >> /etc/gitlab/gitlab.rb"
...
curl -s --header "PRIVATE-TOKEN: $personal_access_token" -d "email=developer@devnetsandbox.cisco.com&password=C1sco12345&username=developer&name=developer&skip_confirmation=true" "http://127.0.0.1/api/v4/users" 2>&1 >> gitlab_setup.log
success
